				...
				successCallback:
				{
					fn: function (resp)
					{
						if( resp.json.masterData && resp.json.masterData.length > 0 )
						{
							...
							else if( resp.json.masterData[0].type == "external" )
							{
								this.options.url = '';
								if( resp.json.masterData[0].url && resp.json.masterData[0].url != '' )
								{
									this.options.url = Alfresco.constants.PROXY_URI 
											+ "api/alvex/external-masterData"
											+ "?url=" + encodeURIComponent(resp.json.masterData[0].url)
											+ "&root=" + encodeURIComponent(resp.json.masterData[0].root)
											+ "&label=" + encodeURIComponent(resp.json.masterData[0].label)
											+ "&value=" + encodeURIComponent(resp.json.masterData[0].value);
								}
								if( ! resp.json.masterData[0].root.toLowerCase().match('^xpath:') )
								{
									this.options.root = resp.json.masterData[0].root;
									this.options.label = resp.json.masterData[0].label;
									this.options.value = resp.json.masterData[0].value;
								}
								else
								{
									this.options.root = '';
					...
					fn: function (resp)
					{
						if( ! resp.json )
						{
							Alfresco.util.PopupManager.displayMessage(
								{ text: this.msg("alvex.masterData.select.externalMasterDataUnavailable") });
							this.renderTextField();
							return;
						}
						var me = this;
